 ...
 *       "result_value": 12.5,
 *       "result_unit": "秒",
 *       "video_s3_url": "s3://px-test-videos/device-001/2024/01/01/exam-123456.mp4",  // 新增
 *       "hls_stream_name": "exam-123456",      // 保留向下相容
  ...
  try {
    ...
    for (const result of results) {
      try {
        ...
        // Upsert 測試結果（新增 video_s3_url 支援）
        const { data, error } = await supabase
          ...
          .upsert({
            ...
            // 新增：S3 影片 URL（優先）
            video_s3_url: result.video_s3_url || null,
            // 保留：HLS 串流（向下相容）
            hls_stream_name: result.hls_stream_name || null,
            hls_start_time: result.hls_start_time || null,
            hls_end_time: result.hls_end_time || null,
