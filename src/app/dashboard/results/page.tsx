...
function ResultCard({ 
  ...
  // 檢查是否有影片（S3 優先，HLS 次之）
  const hasVideo = !!(result.video_s3_url || result.hls_stream_name)
...
export default function ResultsPage() {
  ...
  // 檢查是否有影片
  const hasVideo = (result: TestResult) => !!(result.video_s3_url || result.hls_stream_name)
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <div className="text-sm text-gray-500">影片來源</div>
              <div className="truncate text-sm">
                {result.video_s3_url ? 'S3' : result.hls_stream_name ? 'HLS' : '-'}
              </div>
            </div>
          </div>
          ...
          {result.video_s3_url && (
            <div>
              <div className="text-sm text-gray-500 mb-1">S3 影片路徑</div>
    ...
    const fetchVideoUrl = async () => {
      try {
        ...
        if (result.video_s3_url) {
          console.log('Using S3 video:', result.video_s3_url)
          
          const response = await fetch('/api/video/s3', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              videoS3Url: result.video_s3_url,
            }),
          })
        ...
        } else if (result.hls_stream_name) {
          // 向下相容：使用 HLS
          console.log('Fallback to HLS:', result.hls_stream_name)
        ...
        console.error('Fetch video URL error:', err)
            ...
            {error && (
              <div className="text-center text-white p-4">
                ...
                <div className="text-sm text-gray-400">
                  {result.video_s3_url ? `S3: ${result.video_s3_url}` : `HLS: ${result.hls_stream_name}`}
                </div>
              </div>
            )}
